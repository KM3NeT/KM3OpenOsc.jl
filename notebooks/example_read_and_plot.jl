...
# ╔═╡ 6f33656f-5c2a-4c49-bf7e-19253a09f3e2
using CairoMakie, FHist

# ╔═╡ 5406dbf8-8880-4739-96eb-f57d909f15b5
md"# KM3NeT Open Test Data

This notebook demonstrates read and fill histograms from the ORCA 433 kt-y oscillations data release from the KM3NeT neutrino telescope using KM3NeT's tools.
"

# ╔═╡ ca9d6ea2-5bef-47ba-beb7-265ad4ff14b1
...
- `KM3NeTTestData`: For accessing test data
- `FHist`: For histogram manipulation
- `CairoMakie`: For later plotting
"

# ╔═╡ 039f0379-7a15-4310-8c68-119ef19e8c17
md"## Loading Data Files
...
# ╔═╡ f1798ae2-3ba6-48a8-a6c2-418be6bda693
begin
...
# ╔═╡ c8b08c5a-625e-402d-ad9d-866c9a54e8bd
md"## Reading the Oscillation Data File
...
# ╔═╡ 7bde3d67-69d4-4946-89bc-199c38a52c71
f = KM3io.OSCFile(OSCFILE)

# ╔═╡ f5344b82-69fd-46a6-b9ca-e440936e6e37
begin
...
# ╔═╡ 604e10f0-3015-4a0b-abb7-aaabca7aa611
md"## Creating Histogram Containers
...
# ╔═╡ f6825f5e-c145-4019-954a-570f365ab5d9
begin
...
# ╔═╡ 78f3e741-c9e4-460f-8bf7-d789de012116
fieldnames(typeof(hn))

# ╔═╡ fe274f77-7862-43ff-8417-5dca7def1da6
keys(hn.hists_reco)

# ╔═╡ f1371af5-7e1c-4e24-8151-487e0a0e8c1b
hn.hists_reco["recoShowers"]

# ╔═╡ 9d70ac3c-1fef-491c-a383-ccfcd5d5d611
md"## Setting Oscillation Parameters
...
These parameters are used to calculate the oscillation matrices (U and H).
It uses `Neurthino.jl` behind the scenes.
"

# ╔═╡ 3fc69748-ec45-4dcf-ac00-3397a85bae89
begin
...
# ╔═╡ 4673b4f5-b88d-4140-a682-41ff70de376d
md"
## Loading Neutrino Flux Model

We load the Honda Frejus site flux model, which provides the atmospheric neutrino flux predictions needed for the analysis.
Any Honda formatted table can be read using this function. It uses `NuFlux.jl` behind the scenes.
"

# ╔═╡ 110ed5c3-eb44-4ab9-ba6b-1994b3fdb9c1
begin
...
# ╔═╡ a9447b03-24e6-4b0d-97a9-a6bebc1f3687
md"## Filling Histograms with Events
...
# ╔═╡ 1b1850a0-28a6-447b-9cfe-55460029fd81
begin
...
# ╔═╡ 8ea80314-e937-4033-a8ea-18b7d52e9506
hn.hists_true["muon_cc_nu"]

# ╔═╡ 26f93fa5-521c-480c-8302-583d4aac3aa1
hn.hists_reco["recoShowers"]

# ╔═╡ b01f071c-4207-4b9e-9e93-26270c26fb26
md"## Exporting results to hdf5
...
# ╔═╡ a34abfa4-c524-4dd8-a62c-c4e8e0fa2d63
begin
...
# ╔═╡ f4770e38-4915-468b-88e0-00b5a6fe8125
md"## Visualizing the Reconstruction Channels

CAVEAT: These plots are done with a test data sample, meaning it does not contain all events which will render the distributions to be different from the actual analysis.
...
# ╔═╡ b4b0032f-827b-40f1-a43d-e109c769f84a
...
# ╔═╡ f9cffe87-0974-4bf1-ae49-92d741b86908
md"## Examining True Event Categories

CAVEAT: These plots are done with a test data sample, meaning it does not contain all events which will render the distributions to be different from the actual analysis.

This plot shows the distribution of true Monte Carlo event categories:
1. muon\_cc\_nu: Charged-current muon neutrino events
2. muon\_cc\_nub: Charged-current muon antineutrino events
"

# ╔═╡ c3fa4649-2345-41ae-b592-542c74a83f3d
with_theme(ATLASTHEME) do
...
KM3OpenOsc = "f3e63bb9-bba0-4fef-92d7-12bec46c3825"
KM3io = "2cab5852-6f21-4982-99b0-6a4792870cfb"
NuFlux = "efbbe403-48f9-4d1c-bbe5-17d2c19d1ebf"
...
manifest_format = "2.0"
project_hash = "0857e1ce320e6f1a019c93ef0500c4347ade6989"
...
[[deps.Corpuscles]]
deps = ["DelimitedFiles", "DocStringExtensions", "InteractiveUtils", "Printf", "Unitful", "UnitfulAtomic"]
git-tree-sha1 = "a22c98492ccd8952391ca67d831bf25ec6d39778"
...
git-tree-sha1 = "3d05a37a8d40524752524da392ec0c2fbee32543"
...
git-tree-sha1 = "b1720a404d282f32b5534590af78444fc23353fe"
...
[[deps.JSON_C_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl"]
git-tree-sha1 = "958458be07605d9f45c306ffe647056f5c36a0f0"
uuid = "9cdfc4e7-e793-5089-b6f7-569a57a60f0a"
version = "0.17.0+0"
...
[[deps.KM3NeTTestData]]
git-tree-sha1 = "ff09889d45e3fcf4b3a54ae1c758dafbcc3e84d2"
uuid = "3249b543-581e-4f22-b7da-6c2cdf549b24"
version = "0.4.21"

[[deps.KM3OpenOsc]]
deps = ["Corpuscles", "DocStringExtensions", "FHist", "HDF5", "JSON", "KM3NeTTestData", "KM3io", "Neurthino", "NuFlux", "UnROOT"]
git-tree-sha1 = "488a39181154bc865c171f748e47dc9a7bc82975"
uuid = "f3e63bb9-bba0-4fef-92d7-12bec46c3825"
version = "0.1.9"

[[deps.KM3io]]
deps = ["Corpuscles", "Dates", "DocStringExtensions", "HDF5", "JSON", "LinearAlgebra", "Printf", "Requires", "Sockets", "StaticArrays", "Statistics", "TOML", "UUIDs", "UnROOT"]
git-tree-sha1 = "1b18aac4d6a5786f38681f98a6a44c30d3313bc2"
uuid = "2cab5852-6f21-4982-99b0-6a4792870cfb"
version = "0.18.6"

    [deps.KM3io.extensions]
    KM3ioKM3DBExt = "KM3DB"
...
[[deps.Neurthino]]
deps = ["AxisArrays", "DocStringExtensions", "LRUCache", "LightGraphs", "LinearAlgebra", "Polynomials", "SparseArrays", "StaticArrays"]
git-tree-sha1 = "cf4e0911547cb085265c56ac615ca0868063dfb5"
...
git-tree-sha1 = "17dc1e55e06f11d5a380fbadea8bb07c1fe8ad7f"
...
[[deps.UnROOT]]
deps = ["AbstractTrees", "Accessors", "ArraysOfArrays", "BitIntegers", "CodecLz4", "CodecXz", "CodecZstd", "FHist", "HTTP", "IterTools", "LRUCache", "LibDeflate", "LorentzVectors", "Memoization", "Mixers", "Mmap", "Parameters", "PrecompileTools", "PrettyTables", "SentinelArrays", "StaticArrays", "StructArrays", "TOML", "Tables", "XRootD", "XXHashNative"]
git-tree-sha1 = "f019e1ff790563b20a219b2367b29cf172543797"
...
[[deps.XRootD]]
deps = ["CxxWrap", "Libdl", "XRootD_cxxwrap_jll", "XRootD_jll"]
git-tree-sha1 = "3c35771d40024c3850be716198d288a015ea0eaa"
uuid = "164e3b87-dc46-4cbc-97a6-5b310108fce0"
version = "0.2.2"

[[deps.XRootD_cxxwrap_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl", "XRootD_jll", "libcxxwrap_julia_jll"]
git-tree-sha1 = "c080fc03ac8e22a8714fdc140b105a85a6d8e41c"
uuid = "bb14ce0c-f071-543a-8ca1-cd3dc3e29ed0"
version = "0.2.0+0"

[[deps.XRootD_jll]]
deps = ["Artifacts", "CompilerSupportLibraries_jll", "JLLWrappers", "JSON_C_jll", "Libdl", "Libuuid_jll", "OpenSSL_jll", "XML2_jll", "Zlib_jll", "libxcrypt_legacy_jll"]
git-tree-sha1 = "4df4c72e5c0cbc941f68a3d7870d7cd0b4cd0683"
uuid = "b6113df7-b24e-50c0-846f-35a2e36cb9d5"
version = "5.7.2+0"
...
deps = ["Artifacts", "JLLWrappers", "Libdl"]
git-tree-sha1 = "70091a11c756455c15bc6380ea000c16f38f2d3e"
uuid = "3eaa8342-bff7-56a5-9981-c04077f7cee7"
version = "0.13.4+0"

[[deps.libdeflate_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl"]
git-tree-sha1 = "6b8184c78d6328e015ae33766c2cc340f18f18ff"
...
[[deps.libxcrypt_legacy_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl"]
git-tree-sha1 = "3815a384f6f3cbde4e64fd23fad35f09b83cb2a8"
uuid = "5ef642bb-a58b-5208-ae37-583168b2c491"
version = "4.4.38+0"
